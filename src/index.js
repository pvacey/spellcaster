...
router.get('/callback', async ({ query: { code } }, env) => {
	try {
		...
		const user_guild_response = await fetch('https://discord.com/api/v10/users/@me/guilds', {
			headers: {
				'Content-Type': 'application/json',
				Authorization: `Bearer ${access_token}`,
			},
		...
		redirect_response.headers.append('Set-Cookie', `user_id=${cookieSigner.sign(user_id, env.DISCORD_CLIENT_SECRET)}; ${cookieSettings}`);
		redirect_response.headers.append(
			'Set-Cookie',
			`servers=${cookieSigner.sign(authorizedGuildIDs, env.DISCORD_CLIENT_SECRET)}; ${cookieSettings}`
		);
 ...
 */
router.get('/deck/:deckID', withCookies, withContent, requireAuth, async ({ params: { deckID } }) => {
	try {
